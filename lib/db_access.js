...
class DbAccess {
  ...
  async end() {
    try {
      ...
      err = new Error(`Error ending connection to database:\n ${err.message}`);
      throw err;
    }
  }
  ...
  async #runQuery(queryString, values = []) {
    ...
    try {
      ...
      err = new Error(`Error running query on DB:\n ${err.message}`);
      throw err;
    } finally {
      if (client) {
        client.release();
  ...
  async getEmployeeByManager(managerId) {
    // If managerId is null, get employees without a manager
    // Otherwise, get employees with the specified manager
    const whereCondition = managerId
      ? "WHERE e.manager_id = $1"
      : "WHERE e.manager_id IS NULL";
    const queryParam = managerId ? [managerId] : [];
    ...
    return await this.#runQuery(queryString, queryParam);
  }
