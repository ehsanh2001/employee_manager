...
class ViewHandler extends Handler {
  constructor(dbAccess) {
    super(dbAccess);
    this.viewActions = {
      "View All Employees": async () => this.getAllEmployees(),
  ...
  async getAllEmployees() {
    let employees = await this.dbAccess.getAllEmployees();
    // If there is no manager, change the value to "No Manager"
    // The manager field is the concatenation of first name a space and last name
    // so if there is no manager, the field will be one space
    employees = employees.map((employee) => {
      return {
        ...employee,
        manager: employee.manager.trim() || "No Manager",
      };
    });
    return employees;
  }
